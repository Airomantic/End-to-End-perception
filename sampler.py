...
def sample(v0, Kappa, T0, N0, tt, M, possibility = None):
    ...
    v_selections = (np.random.rand(M) >= 0.2).astype(int) #0 and 1
    velocities = v_options[v_selections, np.arange(M)]  # v0 or 15*random

    # generate longitudinal distances
    # tt (41,)
    L = velocities[:, None] * tt[None, :] + accelerations[:, None] * (tt[None, :]**2) / 2 #路程
    L_straight = L[:straight_num] #120
    L = L[straight_num:] #480,41
    # print("L:", L)
    ...
    line_points = L_straight[:, :, None] * T0[None, None, :] # (120,41)* [0,1] = (120,41,2)
    ...
    radius = np.abs(1 / Krappa) # R
    center = np.array([-1 / Krappa, 0]) # -R, 0
    circle_phis = L / radius if Krappa >= 0 else np.pi - L/radius #直接使用弧长与半径的比值来计算角度 (480,41)
